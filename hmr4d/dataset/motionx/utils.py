...
def transform_keypoints_linear(kp_2d, bbox, patch_width, patch_height):
    kpt_num = kp_2d.shape[1]
    center_x = bbox[:,0] 
    center_y = bbox[:,1] 
    scale = bbox[:,2] 
    width = height = scale * 200
    ...
    kp_2d[:,:,0] = kp_2d[:,:,0]-np.repeat(x0[:,None], repeats=kpt_num, axis=1)
    kp_2d[:,:,1] = kp_2d[:,:,1]-np.repeat(y0[:,None], repeats=kpt_num, axis=1)

    kp_2d[:,:,0] = kp_2d[:,:,0]/np.repeat(width[:,None], repeats=kpt_num, axis=1)*patch_width
    kp_2d[:,:,1] = kp_2d[:,:,1]/np.repeat(height[:,None], repeats=kpt_num, axis=1)*patch_height
    return kp_2d
def bbox_normalization_linear(kp_2d, bbox, patch_width, patch_height):
    to_torch = False
    if isinstance(kp_2d, torch.Tensor):
        to_torch = True
        kp_2d = kp_2d.numpy()
    ...
    out_kp_2d = transform_keypoints_linear(kp_2d, bbox, patch_width, patch_height)
    norm_kp_2d = normalize_keypoints_to_patch(out_kp_2d, patch_width)
...
def normalize_kp_2d(kp_2d, patch_width=224, patch_height=224, bbox=None, mask=None):
    """_summary_
    ...
    """
    if bbox is None:
        bbox = compute_bbox_from_keypoints(kp_2d, do_augment=False, mask=mask)

    norm_kp_2d, out_kp_2d = bbox_normalization(kp_2d, bbox, patch_width, patch_height)
    return norm_kp_2d, out_kp_2d, bbox

def normalize_kp_2d_linear(kp_2d, patch_width=224, patch_height=224, bbox=None, mask=None):
    """_summary_
        kpts from [-1, 1] (not extactly)
    ...
    norm_kp_2d, out_kp_2d = bbox_normalization_linear(kp_2d, bbox, patch_width, patch_height)
    return norm_kp_2d, out_kp_2d, bbox
def normalize_kp_2d_linear2(kp_2d, patch_width=224, patch_height=224, bbox=None, mask=None, not_moving=False, multiview=False, randselect=True, scale=None):
    """_summary_
        kpts from [-1, 1] (not extactly)
    ...
    if multiview:
        ...
        out_kp_2d = []
        for i in range(kp_2d.shape[0]):
            norm_kp_2d_i, out_kp_2d_i = bbox_normalization_linear(kp_2d[i], bbox, patch_width, patch_height)

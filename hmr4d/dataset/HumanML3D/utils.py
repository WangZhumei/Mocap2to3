...
def load_motion_files2(path, humanact_path, index_path):
    index_file = pd.read_csv(index_path)
    source_paths = []
    new_names = []
    start_frames = []
    ...
    for i in range(index_file.shape[0]):
        ...
        end_frame = index_file.loc[i]["end_frame"]
        if "humanact12" in source_path:
            source_path = source_path.replace("./pose_data/humanact12/humanact12", humanact_path)
        else:
            source_path = source_path.replace("./pose_data", path)
            source_path = source_path.replace(".npy", ".npz")
        source_paths.append(source_path)
...
def swap_left_right(data):
    ...
    data[:, left_chain] = tmp
    return data
def swap_left_right_coco(data):
    # ignore hands
    assert len(data.shape) == 3 and data.shape[-1] == 3
    data = data.clone()
    data[..., 0] *= -1
    right_chain = [12, 14, 16, 6, 8, 10, 2, 4]
    left_chain = [11, 13, 15, 5, 7, 9, 1, 3]
    tmp = data[:, right_chain]
    data[:, right_chain] = data[:, left_chain]
    data[:, left_chain] = tmp
    return data

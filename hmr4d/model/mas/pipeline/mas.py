...
class MASPipeline(nn.Module, PipelineHelper):
    ...
    def forward_sample(self, inputs):
        ...
        max_L = inputs["gt_motion"].shape[1]  
        ...
        for i, t in enumerate(timesteps):
            ...
            if i % self.record_interval == 0:
                x0_ori_space = self.decoder_motion2d(x0_)  
                ...
                pred_progress["pred_motion2d"].append(x0_ori_space_) 
    ...
    def forward_sample3d(self, inputs):
        ...
        def cvt_x_to_c_p2d(x):
            try:
                ...
                normed_p2d = torch.cat([torch.zeros_like(normed_p2d[..., :1, :]), normed_p2d], dim=-2) 
        ...
        f_text = repeat(f_text, "b d -> (b v) d", v=V)  
        f_condition = {
            "f_text": f_text,  # (BV, D)
        }
        ...
        for i, t in enumerate(timesteps):
            # 1. Denoiser + Sampler.step
            if self.args.is_camemb:
                f_cam = rearrange(inputs["RT_emb"], "b v d -> (b v) d", v=V) 
                f_condition["f_RT"] = f_cam  # (BV, C)
            if self.args.is_Kemb:
                f_K = rearrange(inputs["K_emb"], "b v d -> (b v) d", v=V)  
                f_condition["f_K"] = f_K  # (BV, C)
            ...
            view_noise, _ = get_view_noise(generator, self.args.is_worldnoise) 

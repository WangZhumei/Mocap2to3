...
import math
import random
def compute_T_move(radius,inverse=False):

    R_ = torch.eye(3).double()#.type_as(T_ayfz2eay)
    Y = math.radians(random.randint(0, 360))
    r_ = random.uniform(0, radius)
    z = r_*math.cos(Y)
    x = r_*math.sin(Y)
    t_ = torch.zeros(3).double()#.type_as(T_ayfz2eay)
    t_[0] = x#.type_as(T_ayfz2eay)
    t_[-1] = z#.type_as(T_ayfz2eay)
    R_rotY = R_[None]
    t_rotY = t_[None]
    if inverse:
        R_rotY_ = R_rotY.transpose(1, 2)  # (B, 3, 3)
        t_rotY_ = -einsum(R_rotY, t_rotY, "b i j , b i -> b j")  # (B, 3)
        return transform_mat(R_rotY_, t_rotY_).float()
    else:
        return transform_mat(R_rotY, t_rotY).float()

def compute_T_move2(distance_,scale,Y_r,inverse=False):
    #rot = []
    #for i in range(4):
    #    rot.append(math.degrees(Y_r[i]))
    R_ = torch.eye(3).double()#.type_as(T_ayfz2eay)
    Y = math.radians(random.randint(-180, 180))
    #print(math.degrees(Y))
    Y_r = np.asarray(Y_r)
    idx = (np.abs(Y_r - Y)).argmin()
    radius = distance_[idx]*scale
    r_ = random.uniform(0, radius)
    z = r_*math.cos(Y)
    x = r_*math.sin(Y)
    t_ = torch.zeros(3).double()#.type_as(T_ayfz2eay)
...
def compute_T_rotY(inverse=False):

    Y = math.radians(random.randint(0, 360))
    R_obj_recovered = R.from_euler('YXZ', [Y, 0,0], degrees=False)
    rotation_matrix_a = R_obj_recovered.as_matrix()
    t_ = torch.zeros(3).double()#.type_as(T_ayfz2eay)
    R_ = torch.tensor(rotation_matrix_a).double()#.type_as(T_ayfz2eay)
    R_rotY = R_[None]
    t_rotY = t_[None]
    if inverse:
        R_rotY_ = R_rotY.transpose(1, 2)  # (B, 3, 3)
        ...
        return transform_mat(R_rotY, t_rotY).float()
def compute_T_eay2ayfz(cam_mat, inverse=False):
    """
    Args:
    ...
    Returns:
        if inverse == False:
           T_af2az: (B, 4, 4)
            ...
            T_az2af: (B, 4, 4)
    """
    t_ = cam_mat[0:3, -1].detach().clone()
    t_[1] = 0  # do not modify z
    r = R.from_matrix(cam_mat.numpy()[:3, :3])
    pitch, roll ,yaw= r.as_euler('YXZ', degrees=False)
    #print(np.degrees(pitch))
    roat = (np.pi - abs(pitch))*np.sign(pitch)*-1
    #print(np.degrees(roat))
    R_obj_recovered = R.from_euler('YXZ', [roat, 0,0], degrees=False)
    rotation_matrix_a = R_obj_recovered.as_matrix()
    R_ = torch.tensor(rotation_matrix_a).type_as(t_)
    
    R_eay2ayfz = R_[None]
    t_eay2ayfz = t_[None]
    if inverse:
        R_ayfz2eay = R_eay2ayfz.transpose(1, 2)  # (B, 3, 3)
        t_ayfz2eay = -einsum(R_eay2ayfz, t_eay2ayfz, "b i j , b i -> b j")  # (B, 3)
        return transform_mat(R_ayfz2eay, t_ayfz2eay)
    else:
        return transform_mat(R_eay2ayfz, t_eay2ayfz)
...
def convert_bbx_xys_to_lurb(bbx_xys):
    ...
    return lurb
def compute_T_move0(joints, inverse=False):
    """
    Args:
        joints: (B, J, 3), in the start-frame, az-coordinate
    Returns:
    ...
    t_rotY = joints[:, 0, :].detach().clone()
    t_rotY[:, 1] = 0  # do not modify y
   
    R_ = torch.eye(3).type_as(t_rotY)
    R_rotY = R_[None]

    if inverse:
        R_rotY_ = R_rotY.transpose(1, 2)  # (B, 3, 3)
        t_rotY_ = -einsum(R_rotY, t_rotY, "b i j , b i -> b j")  # (B, 3)
        return transform_mat(R_rotY_, t_rotY_)[0]
    else:
        return transform_mat(R_rotY, t_rotY)[0]
def compute_T_ayfx(joints, inverse=False):
    """
    Args:
        joints: (B, J, 3), in the start-frame, az-coordinate
    Returns:
    ...
    angle = math.atan(RL_xz[0,0]/RL_xz[0,1])
    R_obj_recovered = R.from_euler('YXZ', [angle, 0,0], degrees=False)
    rotation_matrix_a = R_obj_recovered.as_matrix()
    R_ = torch.tensor(rotation_matrix_a).type_as(t_rotY)
    R_rotY = R_[None]

    if inverse:
        R_rotY_ = R_rotY.transpose(1, 2)  # (B, 3, 3)
        t_rotY_ = -einsum(R_rotY, t_rotY, "b i j , b i -> b j")  # (B, 3)
        return transform_mat(R_rotY_, t_rotY_)[0],angle
    else:
        return transform_mat(R_rotY, t_rotY)[0],angle

def compute_T_ayfz(T,angle, inverse=False):
    """
    Args:
        joints: (B, J, 3), in the start-frame, az-coordinate
    Returns:
    ...
    R_obj_recovered = R.from_euler('YXZ', [angle, 0,0], degrees=False)
    rotation_matrix_a = R_obj_recovered.as_matrix()
    R_ = torch.tensor(rotation_matrix_a).type_as(T)
    R_rotY = R_[None]
    t_rotY = t_[None]
    ...
    if inverse:
        R_rotY_ = R_rotY.transpose(1, 2)  # (B, 3, 3)
        t_rotY_ = -einsum(R_rotY, t_rotY, "b i j , b i -> b j")  # (B, 3)
        return transform_mat(R_rotY_, t_rotY_)
    else:
        return transform_mat(R_rotY, t_rotY_)

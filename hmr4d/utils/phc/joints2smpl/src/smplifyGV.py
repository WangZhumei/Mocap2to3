        ...
        if self.use_lbfgs:
            ...
            for i in range(self.num_iters):
                ...
                def closure():
                    ...
                    loss = camera_fitting_loss(
                        model_joints[:, self.smpl_index], camera_translation, init_cam_t, camera_center,j2d[:, self.corr_index],conf_2d[:, self.corr_index],focal_length
                    )
                    loss.backward()
                    return loss
            ...
            for i in range(self.num_iters):
                ...
                loss = camera_fitting_loss(
                    ...
                    j2d[:, self.corr_index],conf_2d[:, self.corr_index],focal_length
                )
                camera_optimizer.zero_grad()
                loss.backward()

...
def get_TRS(rot_mat, pos):
    ...
    if isinstance(rot_mat, torch.Tensor):
        mat = torch.eye(4, device=pos.device).repeat(pos.shape[:-1] + (1, 1)) #4, 4, 4
...
def rot_matrix_from_quaternion(q):
    ...
    R0 = torch.stack([R00, R01, R02], dim=-1) #4->4 3
    ...
    return R
    '''
    tensor([[[ 0.3426,  0.0000, -0.9395],
         [ 0.0000,  1.0000,  0.0000],
         [ 0.9395,  0.0000,  0.3426]],

        [[ 0.9395,  0.0000,  0.3426],
         [ 0.0000,  1.0000,  0.0000],
         [-0.3426,  0.0000,  0.9395]],
...
def quat_from_angle_axis(angle, axis):
    theta = (angle / 2).unsqueeze(-1) #4, 1
    xyz = normalize(axis) * torch.sin(theta.clone()) #4, 3
    w = torch.cos(theta.clone()) #4, 1
    return quat_unit(torch.cat([xyz, w], dim=-1)) #4, 4
    '''
    tensor([[ 0.0000,  0.5733,  0.0000, -0.8193],
        [-0.0000, -0.1740, -0.0000, -0.9848],
        [-0.0000, -0.8193, -0.0000, -0.5733],
        [-0.0000, -0.9848, -0.0000,  0.1740]])
    '''

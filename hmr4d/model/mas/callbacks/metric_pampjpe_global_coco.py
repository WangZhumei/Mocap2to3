...
from hmr4d.utils.eval.eval_utils import compute_pampjpe_metrics,compute_pampjpe_t_metrics,compute_wampjpe_metrics,compute_wampjpe_metrics_rte
...
class MetricPAMPJPEGlobalCoCo(pl.Callback):
    def __init__(
        self,
        endecoder_opt,
        top_k=3,
    ...
    def on_predict_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=0):
        ...
        for b in range(B):
            ...
            pa_mpjpe_b,mpjpe_b,_ = compute_pampjpe_t_metrics(gt_ayfz_motion[b,:l], pred_ayfz_motion[b,:l],pelvis_idxs=[11, 12])
            _,_,rte_b = compute_wampjpe_metrics_rte(gt_ayfz_motion[b,:l], pred_ayfz_motion[b,:l])
            pa_mpjpe_b_m.append(pa_mpjpe_b.mean())
            mpjpe_b_m.append(mpjpe_b.mean())
            rte_b_m.append(rte_b.mean())
        ...
        self.rte.update(torch.from_numpy(np.array([np.array(rte_b_m).mean()])))

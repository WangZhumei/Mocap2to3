...
def get_ground_mesh(pos, upaxis="y"):
    ...
    if upaxis == "y":
        ...
        #real word 0 and word in cam 0
        translation = np.array([-5.0, - 1.0, -9.0])
        mesh = o3d.geometry.TriangleMesh.create_box(width=10, height=1, depth=10)
    elif upaxis == "z":
...
class o3d_skeleton_animation:
    def __init__(
        ...
        Args:
            ...
            pos_2d (tensor): (progress, V, T, J, 2) 2d joints positions relative to each camera view 100, 270, 22, 3
            pred_pos (tensor): (progress, N, T, J, 3) 3d joints positions directly predicted by other models.
            gt_pos (tensor): (progress, N, T, J, 3) groudtruth 3d joints positions. 270, 22, 3
        ...
        if gt_pos is not None:
            self._setup_gt_3d_skeleton(gt_pos,upaxis, skeleton_type)
    ...
    def _setup_3d_skeleton(self, pos, upaxis, skeleton_type):
        ...
        self.pos = pos
    def _setup_gt_3d_skeleton(self, pos, upaxis,skeleton_type):
        if isinstance(pos, torch.Tensor):
            pos = torch.clone(pos).cpu().detach().numpy()
        gt_J = pos.shape[-2]
        self.gt_J = gt_J

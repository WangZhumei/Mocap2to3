...
from hmr4d.dataset.HumanML3D.utils import load_motion_files, load_motion_files2,swap_left_right,swap_left_right_coco, upsample_motion
...
class BaseDataset(data.Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.J_regressor_coco = torch.load("hmr4d/utils/body_model/smpl_coco17_J_regressor.pt")
        #self.humanact12_coco = torch.load('inputs/HumanAct12/fit_align_coco.pth')
    ...
    def _load_dataset(self):
        self.motion_files, self.new_names, self.start_frames, self.end_frames = load_motion_files2(
...
class Dataset(BaseDataset):
    ...
    def _load_dataset(self):
        if ".pth" in self.root:
            self.motion_files_coco = torch.load(self.root)
            self.motion_files_smpl = torch.load("inputs/hml3d/joints3d.pth")
        else:
    ...
    def _load_data(self, idx):
        ...
        joints3d_smpl = self.motion_files_smpl[seq_name_]["joints3d"]
        joints3d_coco = self.motion_files_coco[seq_name_]["J_from_verts_coco"]
        joints3d_smpl = joints3d_smpl[start_frame:end_frame]
        joints3d_coco = joints3d_coco[start_frame:end_frame]
        return_data = {"joints3d_smpl": joints3d_smpl, "joints3d_coco": joints3d_coco, "text_list": text_list}
        return return_data
    ...
    def prepare_meta(self, split):
        ...
        with cs.open(split_file, "r") as f:
            for line in f.readlines():
                seq_name = line.strip()
                if seq_name + ".npy" in self.motion_files_smpl.keys():
                    motion = self.motion_files_smpl[seq_name + ".npy"]["joints3d"]
...
class PointMapTrainDataset(BaseDataset):
    def __init__(
        self,
        #root='inputs/hml3d/joints3d_coco.pth',
        min_motion_time=2,
        max_motion_time=10,
        max_text_len=20,
        is_ignore_transl=True,
    ...
    def __len__(self):
        ...
        return len(self.idx2meta)
    def _process_data(self, data, idx):
        joints_pos_smpl = data["joints3d_smpl"]
        joints_pos = data["joints3d_coco"]
        if isinstance(joints_pos, np.ndarray):
            joints_pos = torch.tensor(joints_pos, dtype=torch.float32)
        if isinstance(joints_pos_smpl, np.ndarray):
            joints_pos_smpl = torch.tensor(joints_pos_smpl, dtype=torch.float32)
        ground = torch.min(joints_pos_smpl[:,:,1])
        thick = 0.05
        if ground>0:
        ...
        while True:
            ran_seq_idx = random.randint(0,len(self.aist_cam)-1)
            seq_name = list(self.aist_cam.keys())[ran_seq_idx]
            ...
            while True:
                ...
                T_ayfz2eay = cam_keys[cam_name]['T_ayfz2eay'].float()
                distance_ = cam_keys[cam_name]['distance']
                Y_r = cam_keys[cam_name]['Y_r']
                T_w2c = cam_key[cam_name]['T_w2c']
                K = cam_key[cam_name]['K'].float()
                gt_T_Eayfz2c = cam_key[cam_name]['gt_T_Eayfz2c'].float()
                ...
                if (first_idx<0 and last_idx<0) or first_idx>0 or length-last_idx-1>0 or a_i>20:
                    break
        ...
        return_data = {
            ...
            "gt_motion": ori_joints_pos_.float(),  # (F, 22, 3)
...
class MVRichPointMapDataset(MVCamDataset):
    ...
    def _process_data(self, data, idx):
        ...
        while True:
            ...
            T_ayfz2eay = cam_keys[cam_key0]['T_ayfz2eay'].float()

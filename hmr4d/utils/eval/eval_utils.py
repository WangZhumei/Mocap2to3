...
def compute_pampjpe_t_metrics(target_j3d,pred_j3d,pelvis_idxs=[1, 2], fps=30):
    """
    Args:
        batch (dict): {
            "pred_j3d": (..., J, 3) tensor
    ...
    pred_j3d = pred_j3d.cpu()#.numpy()  # (..., J, 3)
    target_j3d = target_j3d.cpu()#.numpy()

    pred_j3d = torch.reshape(pred_j3d,(-1,pred_j3d.size()[-2],pred_j3d.size()[-1]))
    target_j3d = torch.reshape(target_j3d,(-1,target_j3d.size()[-2],target_j3d.size()[-1]))
    pred_j3d_ori = pred_j3d.clone()
    target_j3d_ori = target_j3d.clone()
    # Align by pelvis
    pred_j3d, target_j3d = batch_align_by_pelvis_2(
        [pred_j3d, target_j3d], pelvis_idxs=pelvis_idxs
    )
    ...
    mpjpe_t = compute_jpe(pred_j3d_ori, target_j3d_ori) * m2mm
    ...
    return pa_mpjpe,mpjpe,mpjpe_t

def compute_wampjpe_metrics(target_j3d_glob,pred_j3d_glob,pelvis_idxs=[1, 2], fps=30):
    """
    Args:
        batch (dict): {
            "pred_j3d": (..., J, 3) tensor
    ...
    pred_j3d_glob = pred_j3d_glob.cpu()#.numpy()  # (..., J, 3)
    target_j3d_glob = target_j3d_glob.cpu()#.numpy()
    ...
    return wa2_mpjpe,waa_mpjpe
def compute_wampjpe_metrics_rte(target_j3d_glob,pred_j3d_glob,pelvis_idxs=[1, 2], fps=30):
    """
    Args:
        batch (dict): {
            "pred_j3d": (..., J, 3) tensor
    ...
    # Additional Metrics
    target_root = (target_j3d_glob[:, 11:12,:]+target_j3d_glob[:, 12:13,:])/2
    pred_root = (pred_j3d_glob[:, 11:12,:]+pred_j3d_glob[:, 12:13,:])/2
    rte = compute_rte(target_root[:, 0].cpu(), pred_root[:, 0].cpu()) * m2mm#1e2
    ...
    #return global_metrics
    return wa2_mpjpe,waa_mpjpe,rte#,jitter

@torch.no_grad()
def compute_pampjpe_metrics(target_j3d,pred_j3d,pelvis_idxs=[1, 2], fps=30):
    """
    Args:
        batch (dict): {
            "pred_j3d": (..., J, 3) tensor
...
def compute_accel_metrics(target_j3d,pred_j3d,pelvis_idxs=[1, 2], fps=30):
    """
    Args:
        batch (dict): {
            "pred_j3d": (..., J, 3) tensor
    ...
    accel = compute_error_accel(joints_pred=pred_j3d.detach().numpy(), joints_gt=target_j3d.detach().numpy(), fps=fps)
...
def compute_jpe(S1, S2):
    ...
    return torch.sqrt(((S1 - S2) ** 2).sum(dim=-1)).mean(dim=-1).detach().numpy()


def compute_perjoint_jpe(S1, S2):
    return torch.sqrt(((S1 - S2) ** 2).sum(dim=-1)).numpy()

def batch_align_by_pelvis_2(data_list, pelvis_idxs=[1, 2]):
    """
...
def compute_jitter(joints, fps=30):
    ...
    return pred_jitter.cpu().detach().numpy() / 10.0

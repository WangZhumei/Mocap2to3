...
class BaseDataset(data.Dataset):
    #def __init__(self,root="inputs/H36M/hmr4d_support/",
    def __init__(self,root="inputs/h36m/joints3d_coco.pth",
        original_coord="az",
        motion_frames=120,  # H36M's videos are 25fps and very long
        lazy_load=False, export_data=False,limit_size=None):
    ...
    def _load_data(self, idx):
        ...
        motion = self.motion_files[mid]
        smpl_params_w = {k: v.clone() for k, v in motion["smpl_params_glob"].items()}

        
        
        data = {
            "smpl_params_w": smpl_params_w,  # (F, 63)
            "vid": mid,  # (F, 3)
        }
...
class Dataset(BaseDataset):
    def __init__(self,root="inputs/h36m/joints3d_coco.pth", limit_size=None):
    ...
    def _get_idx2meta(self):
        ...
        self.idx2meta = []
        #print(len(list(self.motion_files.keys())))
        for vid in self.motion_files:
            seq_length = self.motion_files[vid]["J_from_verts_coco"].shape[0]
        ...
        Log.info(f"[H36M] {len(self.idx2meta)} sequences. ")
    def __len__(self):
        if self.limit_size is not None:
            return min(self.limit_size, len(self.idx2meta))
        return len(self.idx2meta)

...
from hmr4d.dataset.AMASS.utils import to_np,to_tensor,vertices2joints
...
class BaseDataset(data.Dataset):
    #def __init__(self,root="inputs/AMASS/hmr4d_support/", cam_augmentation="v11", limit_size=None,skip_moyo=True,export_data=False,random1024=False):
    def __init__(self,root="inputs/amass/joints3d_coco.pth", cam_augmentation="v11", limit_size=None,skip_moyo=True,export_data=False,random1024=False):
        super().__init__()
        ...
        #self.J_regressor_h36m = np.load('./inputs/J_regressor_h36m.npy')
        #self.J_regressor_h36m = to_tensor(to_np(self.J_regressor_h36m), dtype=torch.float32)
...
class Dataset(BaseDataset):
    def __init__(self,root="inputs/amass/joints3d_coco.pth", limit_size=None):
        self.root = root
        self.limit_size = limit_size
    ...
    def _load_data(self, idx):
        ...
        joints3d = self.motion_files[mid]["J_from_verts_coco"]
    ...
    def _get_idx2meta(self):
        ...
        for vid in self.motion_files:
            ...
            if self.skip_moyo and "moyo_smplxn" in vid:
                continue
            vid_n = vid.replace('/','_')
            seq_length = self.motion_files[vid_n]["J_from_verts_coco"].shape[0]
            ...
            for i in range(slice_):
                ...
                #idx+=1
                self.idx2meta.extend([(vid_n, s_motion,start_,end_)])
            if slice_==0:
                end_ = 0
            if seq_length-1-end_>=min_motion_len:
                #idx+=1
                self.idx2meta.extend([(vid_n, s_motion,end_,seq_length-1)])
        Log.info(f"[AMASS] {len(self.idx2meta)} sequences. ")
    def __len__(self):
        if self.limit_size is not None:
            return min(self.limit_size, len(self.idx2meta))
        return len(self.idx2meta)

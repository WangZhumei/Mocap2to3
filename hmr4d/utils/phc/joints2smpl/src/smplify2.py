...
class SMPLify2D:
    ...
    def __call__(self, init_pose, init_betas, init_cam_t, j2d, conf_2d=1.0, seq_ind=0):
        ...
        if self.use_lbfgs:
            ...
            for i in range(self.num_iters):
                ...
                def closure():
                    ...
                    loss = camera_fitting_loss(
                        model_joints[:, self.smpl_index], camera_translation, init_cam_t, camera_center,j2d[:, self.corr_index],conf_2d[:, self.corr_index]
                    )
                    loss.backward()
                    return loss
            ...
            for i in range(self.num_iters):
                ...
                loss = camera_fitting_loss(
                    ...
                    j2d[:, self.corr_index],conf_2d[:, self.corr_index],
                )
                camera_optimizer.zero_grad()
                loss.backward()

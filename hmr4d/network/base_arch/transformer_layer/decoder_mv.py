...
class MVTransformerDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.pmv_norm = nn.LayerNorm(d_model, eps=layer_norm_eps, **factory_kwargs)
        self.pmv_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout, batch_first=batch_first, **factory_kwargs)
        self.pmv_dropout = nn.Dropout(dropout)
        self.pmv_linear = zero_module(nn.Linear(d_model, d_model))
    ...
    def forward(
        ...
        L, B, V, C = tgt.shape
        _ = tgt_is_causal, memory_is_causal
        ...
        if self.norm_first:
            ...
            memory = memory + self._pmv_attn_block(self.pmv_norm(memory), memory_mask)
            x = rearrange(x, "l b v c -> l (b v) c")
            memory = rearrange(memory, "l b v c -> l (b v) c")
            x = x + self._sa_block(self.norm1(x), tgt_mask, tgt_key_padding_mask)
            x = x + self._mha_block(self.norm2(x), memory, memory_mask, memory_key_padding_mask)
            x = x + self._ff_block(self.norm3(x))
        else:
            x = x + self.mv_norm(self._mv_attn_block(x, tgt_mask))
            memory = memory + self.pmv_norm(self._pmv_attn_block(memory, memory_mask))
            x = rearrange(x, "l b v c -> l (b v) c")
            memory = rearrange(memory, "l b v c -> l (b v) c")
    ...
    def _pmv_attn_block(self, mem: Tensor, attn_mask: Optional[Tensor]) -> Tensor:
        L, B, V, _ = mem.shape
        mem = rearrange(mem, "l b v c -> v (l b) c")
        mem = self.pmv_attn(mem, mem, mem, attn_mask=attn_mask, need_weights=False)[0]
        mem = rearrange(mem, "v (l b) c -> l b v c", l=L, b=B, v=V)
        return self.pmv_linear(self.pmv_dropout(mem))

...
class MV_COCO_FromPredPipeline(nn.Module, PipelineHelper):
    ...
    def forward_sample3d(self, inputs):
        ...
        def cvt_w_p3d_to_x_bbox_wh(w_p3d_c):
            c_p3d_c = apply_T_on_points(
                repeat(w_p3d_c, "b (l j) c -> (b v l) j c", v=V, l=max_L),
                repeat(T_w2c, "b v c d -> (b v l) c d", l=max_L),
            )  
            if len(Ks.size())== 4:
                i_p2d_c = project_p2d(
                    c_p3d_c, repeat(Ks, "b v c d -> (b v l) c d", l=max_L), is_pinhole=is_pinhole
                )  
            elif len(Ks.size())== 5:
                i_p2d_c = project_p2d(
                    c_p3d_c, rearrange(Ks, "b v l c d -> (b v l) c d"), is_pinhole=is_pinhole
                )  
            normed_p2d_c = normalize_keypoints_to_patch(i_p2d_c, crop_size=patch_size)  
            normed_p2d_c = rearrange(normed_p2d_c, "(b v l) j c -> b v l j c", b=B, v=V, l=max_L)

            i_p2d_c = rearrange(i_p2d_c, "(b v l) c d -> b v l c d", b=B, v=V, l=max_L)
            normed_motion2ds_c = []
            bboxs_c = []
            for b in range(B):
                t_norm = []
                t_bbox = []
                for v in range(V):
                    normed_pred_motion2d_c,_,bbox_c =normalize_kp_2d_linear(i_p2d_c[b,v].cpu()) 
                    t_norm.append(normed_pred_motion2d_c)
                    t_bbox.append(bbox_c)
                t_norm = torch.stack(t_norm)
                t_bbox = torch.stack(t_bbox)
                normed_motion2ds_c.append(t_norm)
                bboxs_c.append(t_bbox)
            normed_motion2ds_c = torch.stack(normed_motion2ds_c).to(i_p2d_c.device) 
            bboxs_c = torch.stack(bboxs_c).to(i_p2d_c.device) 

            w_h_c = normalize_keypoints_to_patch(bboxs_c[...,2] * 200, crop_size=patch_size)[...,None,None]
            w_h_c = torch.cat([w_h_c,w_h_c],dim=-1)
            
            normed_motion2ds_c[...,0,:] = normed_p2d_c[...,0,:]
            normed_motion2ds_c = torch.cat([normed_motion2ds_c,w_h_c],dim=-2)


            try:
                x = self.encoder_motion2d(normed_motion2ds_c)  
            except RuntimeError:
                # Separate process zero root
                x = self.encoder_motion2d(normed_motion2ds_c) 
            return x  # (BV, J*2, L)
        ...
        gt_x_g = self.encoder_motion2d_global(gt_motion2d_global)  
        ...
        for i, t in enumerate(timesteps):
            ...
            c_p2d_c = cvt_x_to_c_p2d_c(x0_)  
            gt_c_p2d_c  = cvt_x_to_c_p2d_global_c(gt_x_g)  
            gt_c_p2d_c = cvt_x_to_c_p2d_global_c(gt_x_g) 
            combine_c_p2d_c = copy.deepcopy(c_p2d_c)
            combine_c_p2d_c[:,0,:,:] = gt_c_p2d_c[:,0,:,:]
            combine_c_p2d_c = rearrange(combine_c_p2d_c, "b v (l j) c-> b v l j c", l=max_L,j=19)
            combine_c_p2d_c = combine_c_p2d_c[...,:-1,:]
            combine_c_p2d_c = rearrange(combine_c_p2d_c, "b v l j c-> b v (l j) c", l=max_L,j=19-1)
            w_p3d_c = triangulate_func(T_w2c, combine_c_p2d_c)  
            # for vis
            w_p3d_c_ = rearrange(w_p3d_c, "b (l j) c -> b l j c", l=max_L, c=3)  # (B, L, J, 3)
            c_p2d_c_ = rearrange(c_p2d_c, "b v (l j) c -> b v l j c", v=V, l=max_L, c=2)[...,:-1,:]  # (B, V, L, J, 2)
            gt_c_p2d_c_ = rearrange(gt_c_p2d_c, "b v (l j) c -> b v l j c", v=V, l=max_L, c=2)[...,:-1,:]  # (B, V, L, J, 2)

            
            # *. Update x0_
            if self.args.is_consisblock:
                # Project x0: (B, V, J*2, L)
                x0_ = cvt_w_p3d_to_x_bbox_wh(w_p3d_c)
...
#
            ...
            if i % self.record_interval == 0:
                ...
                pred_progress["pred_motion"].append(w_p3d_c_) 
                gt_c_p2d_c_[...,1,:] = 0
                c_p2d_s = torch.cat([gt_c_p2d_c_[:, :1], c_p2d_c_[:, 1:]], dim=1)
                pred_progress["pred_motion2d"].append(c_p2d_c_[...,1:,:]) 
        ...
        # Post-processing
        outputs["pred_motion"] =w_p3d_c_[...,1:,:] 
        outputs["pred_global_motion"] =w_p3d_c_[...,1:,:]
